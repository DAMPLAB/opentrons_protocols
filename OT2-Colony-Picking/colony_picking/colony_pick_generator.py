...
def main():
	...
	plates = generate_plates(preprocessed_image_filenames, source_plate_filename, config['plate_locations'])
...
def ask_source_plate_filename():
	...
	source_plate_filename = filedialog.askopenfilename(title = "Select plate map", filetypes = (("CSV files","*.CSV"),("all files","*.*")))
...
def generate_plates(preprocessed_image_filenames, source_plate_filename, plate_locations):
...
def draw_previews(opencfu_outputs, preview_path):
	for image_filename, opencfu_output in opencfu_outputs.items():
		...
		preview_filename = preview_path + '/preview_colonies_' + os.path.basename(image_filename)
		# Save image preview
		colonies_image.save(preview_filename)
		return preview_filename

# Draws the colony regions and saves output in temp folder.
def draw_regions(preview_path, preview_filename, plate_location, rotate, pixels_per_mm, colony_regions, plate_origin):
	# Open source image
	...
	# Save
	new_preview_filename = preview_path + '/preview_regions_' + os.path.basename(preview_filename)
